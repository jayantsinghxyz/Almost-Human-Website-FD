        ...
        <div className="bg-transparent p-4 sm:p-6 md:p-8">
          ...
          <div className="relative">
            ...
            containScroll: false
              ...
              <CarouselPrevious className="fixed left-4 md:left-8 top-1/2 -translate-y-1/2 z-50 h-10 w-10 md:h-12 md:w-12" />
              <CarouselNext className="fixed right-4 md:right-8 top-1/2 -translate-y-1/2 z-50 h-10 w-10 md:h-12 md:w-12" />
            </Carousel>

            {/* Navigation Dots */}
            <div className="flex justify-center gap-2 mt-6 md:mt-8">
              {PERSONAL_PROJECTS_VIDEOS.map((_, index) => <button key={index} onClick={() => api?.scrollTo(index)} className={`h-2 rounded-full transition-all ${index === current ? "w-8 bg-primary" : "w-2 bg-muted-foreground/30 hover:bg-muted-foreground/50"}`} aria-label={`Go to slide ${index + 1}`} />)}
            </div>
          </div>
        </div>
      </div>
